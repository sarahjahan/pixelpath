...
function GamesSearchPage() {
  ...
  const addGame = async (id, title, coverArt, tags, genres) => {
    try {
      ...
      toast.success(
        `The game you selected was sucessfully added to library. Refreshing Games list.`
      );
    } catch (error) {
  ...
  const delGame = async (id) => {
    try {
      ...
      toast.success(
        `The game you selected was removed from your library. Refreshing Games list.`
      );
    } catch (error) {
